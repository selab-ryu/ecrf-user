...
import ecrf.user.service.persistence.CRFAutoqueryUtil;
...
public class CRFAutoqueryPersistenceImpl
	 ...
	 */
	@Override
	public List<CRFAutoquery> filterFindByGroupId(long groupId) {
	 ...
	 */
	@Override
	public List<CRFAutoquery> filterFindByGroupId(
		long groupId, int start, int end) {
	...
	public List<CRFAutoquery> filterFindByGroupId(
		...
		if (getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_CRFAUTOQUERY_WHERE);
		}
		else {
			sb.append(
				_FILTER_SQL_SELECT_CRFAUTOQUERY_NO_INLINE_DISTINCT_WHERE_1);
		}
		...
		if (!getDB().isSupportsInlineDistinct()) {
			sb.append(
				_FILTER_SQL_SELECT_CRFAUTOQUERY_NO_INLINE_DISTINCT_WHERE_2);
		}
		...
		if (orderByComparator != null) {
			...
			if (getDB().isSupportsInlineDistinct()) {
				...
				sb.append(CRFAutoqueryModelImpl.ORDER_BY_SQL);
			}
		}

		String sql = InlineSQLHelperUtil.replacePermissionCheck(
			sb.toString(), CRFAutoquery.class.getName(),
		...
		try {
			...
			if (getDB().isSupportsInlineDistinct()) {
				sqlQuery.addEntity(
					_FILTER_ENTITY_ALIAS, CRFAutoqueryImpl.class);
			}
			else {
				sqlQuery.addEntity(
					_FILTER_ENTITY_TABLE, CRFAutoqueryImpl.class);
			}
			...
			return (List<CRFAutoquery>)QueryUtil.list(
	...
	protected CRFAutoquery filterGetByGroupId_PrevAndNext(
		Session session, CRFAutoquery crfAutoquery, long groupId,
		OrderByComparator<CRFAutoquery> orderByComparator, boolean previous) {
		...
		if (getDB().isSupportsInlineDistinct()) {
			sqlQuery.addEntity(_FILTER_ENTITY_ALIAS, CRFAutoqueryImpl.class);
		}
		else {
			sqlQuery.addEntity(_FILTER_ENTITY_TABLE, CRFAutoqueryImpl.class);
		}
		...
		List<CRFAutoquery> list = sqlQuery.list();
	...
	public int filterCountByGroupId(long groupId) {
		...
		sb.append(_FILTER_SQL_COUNT_CRFAUTOQUERY_WHERE);
	 ...
	 */
	@Override
	public List<CRFAutoquery> filterFindByG_C(long groupId, long crfId) {
	 ...
	 */
	@Override
	public List<CRFAutoquery> filterFindByG_C(
		long groupId, long crfId, int start, int end) {
	...
	protected CRFAutoquery filterGetByG_C_PrevAndNext(
		Session session, CRFAutoquery crfAutoquery, long groupId, long crfId,
		OrderByComparator<CRFAutoquery> orderByComparator, boolean previous) {
	 ...
	 */
	@Override
	public List<CRFAutoquery> filterFindByG_C_S(
		long groupId, long crfId, long subjectId) {
	...
	protected CRFAutoquery filterGetByG_C_S_PrevAndNext(
		Session session, CRFAutoquery crfAutoquery, long groupId, long crfId,
		long subjectId, OrderByComparator<CRFAutoquery> orderByComparator,
		boolean previous) {
	...
	public void cacheResult(List<CRFAutoquery> crfAutoqueries) {
		if ((_valueObjectFinderCacheListThreshold == 0) ||
			((_valueObjectFinderCacheListThreshold > 0) &&
			 (crfAutoqueries.size() > _valueObjectFinderCacheListThreshold))) {
	...
	public CRFAutoquery updateImpl(CRFAutoquery crfAutoquery) {
		...
		if (isNew && (crfAutoquery.getCreateDate() == null)) {
			if (serviceContext == null) {
				crfAutoquery.setCreateDate(date);
			}
			else {
				crfAutoquery.setCreateDate(serviceContext.getCreateDate(date));
			}
		}

		if (!crfAutoqueryModelImpl.hasSetModifiedDate()) {
			if (serviceContext == null) {
				crfAutoquery.setModifiedDate(date);
			}
			else {
	...
	public void activate() {
		...
		_setCRFAutoqueryUtilPersistence(this);
	}

	@Deactivate
	public void deactivate() {
		_setCRFAutoqueryUtilPersistence(null);
	...
	private void _setCRFAutoqueryUtilPersistence(
		CRFAutoqueryPersistence crfAutoqueryPersistence) {

		try {
			Field field = CRFAutoqueryUtil.class.getDeclaredField(
				"_persistence");

			field.setAccessible(true);

			field.set(null, crfAutoqueryPersistence);
		}
	...
	private static final String _FILTER_SQL_SELECT_CRFAUTOQUERY_WHERE =
		"SELECT DISTINCT {crfAutoquery.*} FROM EC_CRFAutoquery crfAutoquery WHERE ";

	private static final String
		_FILTER_SQL_SELECT_CRFAUTOQUERY_NO_INLINE_DISTINCT_WHERE_1 =
			"SELECT {EC_CRFAutoquery.*} FROM (SELECT DISTINCT crfAutoquery.autoQueryId FROM EC_CRFAutoquery crfAutoquery WHERE ";

	private static final String
		_FILTER_SQL_SELECT_CRFAUTOQUERY_NO_INLINE_DISTINCT_WHERE_2 =
			") TEMP_TABLE INNER JOIN EC_CRFAutoquery ON TEMP_TABLE.autoQueryId = EC_CRFAutoquery.autoQueryId";

	private static final String _FILTER_SQL_COUNT_CRFAUTOQUERY_WHERE =
		"SELECT COUNT(DISTINCT crfAutoquery.autoQueryId) AS COUNT_VALUE FROM EC_CRFAutoquery crfAutoquery WHERE ";

	private static final String _FILTER_ENTITY_ALIAS = "crfAutoquery";

	private static final String _FILTER_ENTITY_TABLE = "EC_CRFAutoquery";

	private static final String _ORDER_BY_ENTITY_ALIAS = "crfAutoquery.";

	private static final String _ORDER_BY_ENTITY_TABLE = "EC_CRFAutoquery.";

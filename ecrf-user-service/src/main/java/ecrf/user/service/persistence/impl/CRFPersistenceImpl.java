...
public class CRFPersistenceImpl
	 ...
	 */
	@Override
	public List<CRF> filterFindByGroupId(long groupId) {
	 ...
	 */
	@Override
	public List<CRF> filterFindByGroupId(long groupId, int start, int end) {
	 ...
	 */
	@Override
	public List<CRF> filterFindByGroupId(
		long groupId, int start, int end,
		OrderByComparator<CRF> orderByComparator) {
		...
		if (getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_CRF_WHERE);
		}
		else {
			sb.append(_FILTER_SQL_SELECT_CRF_NO_INLINE_DISTINCT_WHERE_1);
		}

		sb.append(_FINDER_COLUMN_GROUPID_GROUPID_2);

		if (!getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_CRF_NO_INLINE_DISTINCT_WHERE_2);
		}
		...
		if (orderByComparator != null) {
			...
			if (getDB().isSupportsInlineDistinct()) {
				...
				sb.append(CRFModelImpl.ORDER_BY_SQL);
			}
		}

		String sql = InlineSQLHelperUtil.replacePermissionCheck(
			sb.toString(), CRF.class.getName(),
		...
		try {
			...
			if (getDB().isSupportsInlineDistinct()) {
				sqlQuery.addEntity(_FILTER_ENTITY_ALIAS, CRFImpl.class);
			}
			else {
				sqlQuery.addEntity(_FILTER_ENTITY_TABLE, CRFImpl.class);
			}
			...
			return (List<CRF>)QueryUtil.list(
	...
	public CRF[] filterFindByGroupId_PrevAndNext(
		...
		if (!InlineSQLHelperUtil.isEnabled(groupId)) {
			return findByGroupId_PrevAndNext(crfId, groupId, orderByComparator);
		}
	...
	protected CRF filterGetByGroupId_PrevAndNext(
		Session session, CRF crf, long groupId,
		OrderByComparator<CRF> orderByComparator, boolean previous) {
	...
	public int filterCountByGroupId(long groupId) {
		...
		sb.append(_FILTER_SQL_COUNT_CRF_WHERE);
	...
	private static final String _FILTER_SQL_SELECT_CRF_WHERE =
		"SELECT DISTINCT {crf.*} FROM EC_CRF crf WHERE ";

	private static final String
		_FILTER_SQL_SELECT_CRF_NO_INLINE_DISTINCT_WHERE_1 =
			"SELECT {EC_CRF.*} FROM (SELECT DISTINCT crf.crfId FROM EC_CRF crf WHERE ";

	private static final String
		_FILTER_SQL_SELECT_CRF_NO_INLINE_DISTINCT_WHERE_2 =
			") TEMP_TABLE INNER JOIN EC_CRF ON TEMP_TABLE.crfId = EC_CRF.crfId";

	private static final String _FILTER_SQL_COUNT_CRF_WHERE =
		"SELECT COUNT(DISTINCT crf.crfId) AS COUNT_VALUE FROM EC_CRF crf WHERE ";

	private static final String _FILTER_ENTITY_ALIAS = "crf";

	private static final String _FILTER_ENTITY_TABLE = "EC_CRF";

	private static final String _ORDER_BY_ENTITY_ALIAS = "crf.";

	private static final String _ORDER_BY_ENTITY_TABLE = "EC_CRF.";
